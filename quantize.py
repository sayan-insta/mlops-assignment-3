...
# 4. Improved manual quantization using int8
quant_min, quant_max = -128, 127
max_val = max(abs(coef.max()), abs(coef.min()))  # symmetric quantization

scale = max_val / quant_max
quant_weights = np.round(coef / scale).astype(np.int8)
quant_bias = int(np.round(intercept / scale))
...
# 5. Dequantize
dequant_weights = quant_weights.astype(np.float32) * scale
